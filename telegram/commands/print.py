...
async def print_navigate_pages(event: events.callbackquery.CallbackQuery.Event):
    ...
    action, conversation_id, page = event.data.decode("utf-8").split(":")
    conversation_id = int(conversation_id)
    page = int(page)

    await event.answer()
    response, buttons = await display_paginated_messages(
        conversation_id, telegram_id, page
    )
    await event.edit(response, buttons=buttons, parse_mode="markdown")


async def display_paginated_messages(
    conversation_id: int, telegram_id: int, page: int
) -> Tuple[str, Union[List[Button], None]]:
